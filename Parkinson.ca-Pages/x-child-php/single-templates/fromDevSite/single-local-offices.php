    ...
    <div class="x-main left" role="main">
        ...
        <div class="entry-content content">
					...
					<section class="simple-flex flex-justify-between">
						<div id="left-contact">
							...
							<div class="red-field-labels bot-mar-1em">
								...
								<strong><?php echo $translation ?></strong> <?php echo (types_render_field('general-inquires-email') );?>
								<br>
								...
								$child_posts = toolset_get_related_posts( get_the_ID(),
								 'partner-recognition',
								  array( 
										...
										'return' => 'post_object' ) );
								foreach ($child_posts as $child_post) { ?>
							<section class="simple-grid sg-2col-75-15 bottom-margin-1">
								<div class="grid__item"><span><?php echo (types_render_field( 'partner-text-description', array( "id"=> "$child_post->ID" ) ) );?></span></div>
								<div class="left-to-right-align logo-mobile">
									<?php echo(types_render_field( 'partner-logo', array( "id"=> "$child_post->ID" ) )); ?>
								</div>
							</section>
							<?php } ?>
					...
					<section class="staff-directory">
						<?php
							$child_posts = toolset_get_related_posts( get_the_ID(), 'staff-directory', array( 'query_by_role' => 'parent', 'return' => 'post_object' ) );
							foreach ($child_posts as $child_post) { ?>
							...
							<div class="profile-staff"><!-- in the image conditional section, this class is replaced by profile-text-container -->
								...
								<?php $translation = __('Local:', 'types-field-labels'); ?>
								<?php echo $translation ?> <?php echo (types_render_field('local-phone') );?><br>
								<?php// $translation = __('Extension:', 'types-field-labels'); ?>
								<?php// echo $translation ?> <?php// echo (types_render_field('staff-extension-2', array( "id"=> "$child_post->ID" ) ) );?>
